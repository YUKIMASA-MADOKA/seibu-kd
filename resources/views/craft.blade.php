...
@section('content')
   ...
   <form action="{{ route('update_craft') }}" method="POST">
      ...
      <div style="display:inline;">5を単位に増減する0から100までの値で注入率を設定してください。<input type="submit" class="btn" value="この値で設定する"></div>
      <table class="table">
      <thead>
         <tr>
            <!--th></th-->
      ...
      <tr >
         ...
         <td><input type="number" name="rate{{ $craft->id }}" id="rate{{ $craft->id }}" style="text-align: right" min="0" max="100" step="5" value={{ $craft->injection_rate }} /></td>
      </tr>
      @endforeach
      </tbody>
      </table>
